...
const regex = /export const fetchExamSubjects = async \([\s\S]*?\} catch \(error\) \{\s*console\.error\("Failed to fetch exam subjects:", error\);\s*throw error;\s*\}\s*\};\s*const delay/m;
