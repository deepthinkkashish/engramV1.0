...
export const NavButton: React.FC<NavButtonProps> = React.memo(({ icon: Icon, label, view, currentView, navigateTo, themeColor }) => {
    ...
    return (
        <button onClick={() => navigateTo(view)} className={`flex flex-col items-center p-2 min-w-[64px] rounded-lg transition duration-200 ${isActive ? `text-${themeColor}-600 dark:text-${themeColor}-400 font-semibold` : `text-gray-500 dark:text-gray-500 hover:text-${themeColor}-500 dark:hover:text-${themeColor}-400`}`} disabled={isNavDisabled}>
