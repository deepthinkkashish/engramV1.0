...
import { Shield, XCircle, MessageSquarePlus, X, Loader, Download, Maximize2, RotateCw } from 'lucide-react';
...
export const SourceViewerModal: React.FC<SourceViewerModalProps> = ({ topicId, topicName, subjectName, pageCount, onClose }) => {
    ...
    const handleRotateImage = async (index: number, currentBase64: string, e: React.MouseEvent) => {
        e.stopPropagation();
        ...
        try {
            ...
            const newBase64DataUrl = canvas.toDataURL('image/jpeg', 0.95);
            const newBase64Raw = newBase64DataUrl.replace(/^data:image\/(png|jpeg);base64,/, "");

            // Save back to IDB
            await saveImageToIDB(`source_${topicId}_${index}`, newBase64Raw);

            // Update state
            setImages(prev => {
                const updated = [...prev];
                updated[index] = newBase64DataUrl;
                return updated;
            });
        } catch (e) {
                            ...
                            return items.map((item) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <button 
                                            className="absolute top-2 right-2 bg-black/60 hover:bg-black/80 text-white p-1.5 rounded transition"
                                            onClick={(e) => handleRotateImage(index, src, e)}
                                            title="Rotate Image"
                                        >
                                            <RotateCw size={14} />
                                        </button>
