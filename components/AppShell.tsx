...
export const AppShell: React.FC<AppShellProps> = ({
    ...
    const mainClasses = `flex-1 flex flex-col min-h-0 no-scrollbar ${
        isEdgeToEdge 
            ...
            : `${['subjects', 'home', 'topicList', 'settings', 'podcastSettings', 'appearance', 'tabBarSettings', 'dateTimeSettings', 'soundsNotifications', 'about', 'profile', 'aiFeatures', 'flashcardHub'].includes(currentView) ? 'px-1.5' : 'px-4'} ${currentView === 'settings' ? 'pt-0' : 'pt-4'} overflow-y-auto`
    }`;
    ...
    return (
        ...
        >
            ...
            <div className={`w-full max-w-[501px] md:rounded-3xl md:shadow-2xl overflow-hidden flex flex-col h-[100dvh] md:h-[calc(100vh-2rem)] border-0 md:border dark:border-gray-800 ${mainBgClass} relative`}>
                ...
                {/* Header Logic */}
                {isLoggedIn && !isPodcastOpen && currentView !== 'onboarding' && currentView !== 'chat' && currentView !== 'settings' && currentView !== 'appearance' && currentView !== 'tabBarSettings' && currentView !== 'dateTimeSettings' && currentView !== 'profile' && currentView !== 'soundsNotifications' && currentView !== 'about' && currentView !== 'terms' && currentView !== 'privacy' && currentView !== 'licenses' && currentView !== 'podcastSettings' && (
                    <header 
                ...
                {isLoggedIn && !isPodcastOpen && ['settings', 'tabBarSettings', 'dateTimeSettings', 'soundsNotifications'].includes(currentView) && currentView !== 'appearance' && (
                    <header 
