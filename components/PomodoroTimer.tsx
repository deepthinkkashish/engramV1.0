...
export const PomodoroTimer: React.FC<PomodoroTimerProps> = ({ topicId, topicName, onTimeLogged, themeColor }) => {
    ...
    return (
        <Card className="flex flex-col items-center w-full relative overflow-hidden p-6">
            <div className="flex justify-between items-center w-full mb-2 relative z-10">
                 <div className="flex items-center space-x-2">
                     ...
                     <div className="flex items-center bg-gray-100 dark:bg-gray-700 rounded-full p-0.5">
                        <button 
                            onClick={() => { setShowSoundMenu(!showSoundMenu); setShowSettings(false); }} 
                        ...
                        <button 
                            onClick={() => { setShowSettings(!showSettings); setShowSoundMenu(false); }} 
                 ...
                 {!showSettings && !showSoundMenu && (
                     <button 
                        onClick={() => setMode(mode === 'stopwatch' ? 'pomodoro' : 'stopwatch')}
            ...
            {/* Settings Overlay */}
            {showSettings && (
                <div className="absolute inset-0 bg-white/95 dark:bg-gray-800/95 z-20 flex flex-col items-center justify-center p-6 animate-in fade-in zoom-in-95 duration-200">
                    <div className="w-full flex justify-between items-center mb-6">
                        <h4 className="text-lg font-bold text-gray-800 dark:text-white">Timer Settings</h4>
                        <button onClick={() => setShowSettings(false)} className="text-gray-400 hover:text-gray-600"><X size={20} /></button>
                    </div>
                    
                    <div className="w-full mb-6">
                        <label className="text-xs font-bold text-gray-500 uppercase mb-2 block tracking-wider">Mode</label>
                        <div className="flex bg-gray-100 dark:bg-gray-700 rounded-xl p-1">
                            <button 
                                onClick={() => setMode('stopwatch')} 
                                className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all ${mode === 'stopwatch' ? 'bg-white dark:bg-gray-600 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400'}`}
                            >
                                Stopwatch
                            </button>
                            <button 
                                onClick={() => setMode('pomodoro')} 
                                className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all ${mode === 'pomodoro' ? 'bg-white dark:bg-gray-600 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400'}`}
                            >
                                Pomodoro
                            </button>
                        </div>
                    ...
                    {mode === 'pomodoro' && (
                        <div className="w-full mb-8">
                            <label className="text-xs font-bold text-gray-500 uppercase mb-2 block tracking-wider">Duration (min)</label>
                            <div className="flex items-center space-x-2">
                                {[25, 45, 60].map(val => (
                                    <button 
                                        key={val}
                                        onClick={() => setSessionDuration(val)}
                                        className={`flex-1 py-2 border rounded-lg text-sm font-bold transition ${duration === val ? `border-${themeColor}-500 bg-${themeColor}-50 text-${themeColor}-700 dark:bg-${themeColor}-900/30` : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300'}`}
                                    >
                                        {val}m
                                    </button>
                                ))}
                            ...
                            <div className="mt-3">
                                <input 
                                    ...
                                    className={`w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer accent-${themeColor}-600`}
                                />
                                <div className="text-center mt-2 font-mono font-bold text-gray-700 dark:text-gray-300">{duration} minutes</div>
                            </div>
                        </div>
                    )}
                    ...
                    <button 
                        ...
                        className={`w-full py-3.5 bg-${themeColor}-600 text-white rounded-xl font-bold shadow-lg hover:bg-${themeColor}-700 transition transform active:scale-95`}
                    >
                        Apply Changes
                    </button>
                </div>
