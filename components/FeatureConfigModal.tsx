...
import { getAvailableModels, resolveModelName } from '../services/gemini';
                        ...
                        {showAdvanced && (
                            <div className="mt-2 animate-in fade-in slide-in-from-top-1 bg-gray-50 dark:bg-gray-800 rounded-xl p-2 border border-gray-100 dark:border-gray-700 max-h-60 overflow-y-auto">
                                {advancedModels.map(opt => {
                                    const currentModel = resolveModelName(prefs.model, featureId);
                                    return (
                                    <button
                                        key={opt.id}
                                    ...
                                    >
                                        {opt.label}
                                        {currentModel === opt.id && <Check size={12} />}
                                    </button>
                                )})}
                            </div>
                        )}
