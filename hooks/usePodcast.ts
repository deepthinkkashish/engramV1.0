...
import { getAudioFromIDB, saveAudioToIDB, getTopicBodyFromIDB } from '../services/storage';
...
export const usePodcast = (userId: string, podcastConfig: { language: 'English' | 'Hinglish', voice?: 'Standard' | 'Deep' }) => {
    ...
    const playTopic = useCallback(async (topic: Topic, onUpdateTopic?: (t: Topic) => void) => {
        ...
        try {
            let topicNotes = topic.shortNotes || '';
            ...
            const script = await generatePodcastScript(topic.topicName, context, podcastConfig.language || 'English', targetDuration, 'podcast');
            setGeneratedScript(script);
            
            // Audio Gen - FeatureID: 'podcast'
            setStatus('Synthesizing Voices...');
            const audioData = await generatePodcastAudio(script, 'podcast', podcastConfig.voice || 'Standard');
    ...
    }, [currentTopic, audioSrc, isPlaying, loading, podcastConfig, togglePlay]);
    ...
    const downloadTopic = useCallback(async (topic: Topic, context: string, durationOverride: number, language: string, onSuccess?: (audioData: Blob | string, script: string) => void) => {
        ...
        if (!isRecap && (!context || context.trim() === `Topic: ${topic.topicName}` || context.trim().length < 50)) {
            const body = topic.shortNotes || "";
            finalContext = `Topic: ${topic.topicName}\n${body}`;
        }
        ...
        try {
            const script = await generatePodcastScript(topic.topicName, finalContext, language, finalDuration, 'podcast');
            const audioData = await generatePodcastAudio(script, 'podcast', podcastConfig.voice || 'Standard');
