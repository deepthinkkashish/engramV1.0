...
export const FlashCardDeck: React.FC<{ 
    ...
    const generateCards = async (overrideIds?: string[]) => {
        ...
        const recentFronts = cardHistory.slice(-50).map(c => c.front);
        const avoidInstruction = recentFronts.length > 0 
            ? `\n\nCRITICAL: Do NOT generate questions that are similar to or cover the exact same concepts as these previously generated cards:\n- ${recentFronts.join('\n- ')}` 
            : '';

        const prompt = `Generate ${desiredCount} high-quality, highly specific flashcards based ONLY on the provided notes.
        IMPORTANT CONSTRAINTS:
        1. Create a mix of questions covering different topics from the provided content.
        2. Avoid generic or overly broad questions. Ask specific, detailed questions that test true understanding of the source material.
        3. The front should be a clear, concise concept or question. DO NOT use conversational filler like "According to the notes," or "Based on the text,". Just ask the question directly.
        4. The back should be the detailed explanation or answer directly from the notes.
        5. Use LaTeX for all math expressions (e.g., $E=mc^2$).
        ${persona ? `6. Adhere strictly to the following persona/custom instructions:\n"${persona}"\nUse the googleSearch tool if the instructions require verifying external information or drawing on recent knowledge.` : ''}
        ${avoidInstruction}
