        ...
        <div className="min-h-full bg-orange-50/30 dark:bg-gray-900 pb-[calc(var(--tabbar-height,64px)+env(safe-area-inset-bottom,0px)+24px)] -mx-1.5 transition-colors duration-200 relative">
            ...
            <div className="px-6 flex flex-col items-center -mt-8 pt-4">
                ...
                <div className="flex justify-center items-center space-x-6 mt-3">
                    {/* Left: Longest Streak */}
                    <div className="flex flex-col items-center justify-center px-4 py-2 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800/80">
                        <span className="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-0.5">Max Streak</span>
                        <span className="text-sm font-black text-gray-800 dark:text-gray-200">{streakInfo.longest} <span className="text-[10px] font-bold text-gray-400">d</span></span>
                    </div>

                    {/* Center: Current Streak */}
                    <button 
                        onClick={() => setActiveModal('streak')}
                        className={`flex flex-col items-center justify-center px-5 py-2 rounded-2xl hover:scale-105 transition active:scale-95 cursor-pointer border ${currentStreak > 0 ? 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800/50 shadow-sm shadow-orange-100 dark:shadow-none' : 'bg-gray-50 dark:bg-gray-800/50 border-gray-100 dark:border-gray-800/80'}`}
                    >
                        <div className="flex items-center space-x-1.5">
                            {currentStreak > 0 && <Flame size={14} className="text-orange-500 animate-pulse" fill="currentColor" />}
                            <span className={`text-sm font-black ${currentStreak > 0 ? 'text-orange-600 dark:text-orange-400' : 'text-gray-500 dark:text-gray-400'}`}>
                                {currentStreak} <span className="text-[10px] font-bold uppercase tracking-widest opacity-80">Day{currentStreak !== 1 ? 's' : ''}</span>
                            </span>
                        </div>
                    </button>

                    {/* Right: Total Focus Time */}
                    <div className="flex flex-col items-center justify-center px-4 py-2 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800/80">
                        <span className="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-0.5">Total Focus</span>
                        <span className="text-sm font-black text-gray-800 dark:text-gray-200">
                            {Math.floor(studyLog.reduce((acc, t) => acc + (t.pomodoroTimeMinutes || 0), 0) / 60)}<span className="text-[10px] font-bold text-gray-400 mx-0.5">h</span>{Math.floor(studyLog.reduce((acc, t) => acc + (t.pomodoroTimeMinutes || 0), 0) % 60)}<span className="text-[10px] font-bold text-gray-400 ml-0.5">m</span>
                        </span>
                    </div>
                </div>
            </div>
