...
export const TopicDetailView: React.FC<TopicDetailViewProps> = React.memo(({ topic, userId, navigateTo, onUpdateTopic, themeColor }) => {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            if (lastSavedNotesRef.current !== null && notes !== lastSavedNotesRef.current) {
                handleSave(notes);
            }
        }, 1500); 
        return () => clearTimeout(timer);
    }, [notes, isLoadingBody, topic, handleSave]); 
                    ...
                    <Card className="flex flex-col relative overflow-hidden">
                        ...
                        <div className="flex space-x-2 mt-auto">
                            <button
                                ...
                                className={`flex-1 py-2.5 bg-gradient-to-r from-red-500 to-pink-500 text-white rounded-xl font-bold shadow-sm text-xs flex items-center justify-center hover:opacity-90 transition disabled:opacity-50 border border-transparent`}
                            >
                            ...
                            <button
                                ...
                                className={`flex-1 py-2.5 bg-white dark:bg-gray-800 text-${themeColor}-600 dark:text-${themeColor}-400 border border-gray-200 dark:border-gray-700 rounded-xl font-bold shadow-sm text-xs flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-700 transition disabled:opacity-50`}
                            >
