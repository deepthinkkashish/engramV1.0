...
import { Calendar as CalendarIcon, CheckSquare, LayoutGrid, MapPin, Search, Plus, Clock, Check, Trash2, X, History, ChevronLeft, ChevronRight, BookOpenText, BarChart2, List, ChevronDown } from 'lucide-react';
import { DateTimeSettings, Habit, Topic } from '../types';
...
export const CalendarView: React.FC<ExtraViewProps> = ({ themeColor, settings, studyLog = [], userId }) => {
    // ... CalendarView implementation unchanged ...
    const startDay = settings?.startDayOfWeek || 'sunday';
    const showWeekNums = settings?.showWeekNumbers || false;
    const additionalCal = settings?.additionalCalendar || 'none';
    ...
    // Try to get daily aggregates first, fallback to raw iteration if aggregates are missing
    const dailySessions = useMemo(() => {
        if (userId) {
            const agg = AnalyticsService.getAggregates(userId);
            // Check if we have data for the selected date in aggregates
            if (agg && agg.dailyTopicMinutes[selectedDate]) {
                ...
                return Object.entries(dayData).map(([topicId, minutes]) => {
                    ...
                    return {
                        ...
                        minutes: minutes,
                        id: topicId // Use topicId as key since we aggregated
                    };
                });
            }
        }

        // Fallback: Scan raw studyLog (Slower for large data)
        ...
        <div className="p-4 space-y-4">
            ...
            <Card className="p-4 text-center">
                <div className="flex justify-between items-center mb-4 px-2">
                    <button onClick={() => changeMonth(-1)}><ChevronLeft className="text-gray-400"/></button>
                    <span className="font-bold text-lg text-gray-700 dark:text-gray-200">
                        {currentMonth.toLocaleString('default', { month: 'long', year: 'numeric' })}
                    </span>
                    <button onClick={() => changeMonth(1)}><ChevronRight className="text-gray-400"/></button>
                </div>
                            ...
                            {weeks.map((week, wIdx) => (
                                <React.Fragment key={wIdx}>
                                    {week.map((d, dIdx) => {
                                        ...
                                        const dateStr = new Date(currentYear, currentMonthIdx, d + 1).toISOString().split('T')[0]; // +1 to fix offset
                                        const isSelected = dateStr === selectedDate;
                                        
                                        // Check study presence using aggregates if possible for speed
                                        let hasStudy = false;
                                        if (userId) {
                                            const agg = AnalyticsService.getAggregates(userId);
                                            if (agg && agg.dailyTotalMinutes[dateStr] > 0) {
                                                hasStudy = true;
                                            } else {
                                                // Fallback check
                                        ...
                                        return (
                                            <div 
                                                ...
                                                className={`h-10 rounded-lg flex flex-col items-center justify-center text-sm cursor-pointer transition relative
                                                    ${isSelected ? `bg-${themeColor}-500 text-white shadow-md` : 'hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300'}`}
                                            >
                                                {d}
                                                {hasStudy && !isSelected && (
                ...
                <p className="text-gray-400 text-xs mt-4">
                    {startDay === 'sunday' ? "Starting week on Sunday." : startDay === 'monday' ? "Starting week on Monday." : "Starting week on Saturday."}
                </p>
            </Card>
...
export const TaskView: React.FC<ExtraViewProps> = ({ themeColor, settings }) => {
    // ... TaskView implementation unchanged ...
...
export const PomodoroFullView: React.FC<ExtraViewProps> = ({ themeColor }) => {
    // ... PomodoroFullView implementation unchanged ...
    const [logs, setLogs] = useState<{ date: string; time: string; minutes: number }[]>([]);
    ...
    const handleLog = (minutes: number) => {
        // Updated to use shared utility which handles storage and event dispatch
        logGlobalSession(minutes, "General Focus", "General");
    };
    ...
    return (
        <div className="p-4 space-y-6">
            ...
            <Card className="p-5">
                <h3 className="font-bold text-gray-800 dark:text-white mb-4 flex items-center text-lg border-b border-gray-100 dark:border-gray-700 pb-2">
                    ...
                    {todaysLogs.length > 0 ? (
                        todaysLogs.map((log, idx) => (
                            <div key={idx} className="flex justify-between items-center py-3 border-b border-gray-50 dark:border-gray-800 last:border-0">
                                <span className="text-sm font-medium text-gray-500 dark:text-gray-400">{log.time}</span>
                                <span className={`text-sm font-bold text-${themeColor}-600 dark:text-${themeColor}-400`}>{log.minutes.toFixed(1)} mins</span>
                            </div>
                        ))
                    ) : (
...
export const HabitTrackerView: React.FC<ExtraViewProps> = ({ themeColor, habits = [], onUpdateHabits }) => {
    // ... HabitTrackerView implementation unchanged ...
    ...
    const confirmDelete = (e: React.MouseEvent, id: string) => {
        e.stopPropagation();
        console.debug("[HABITS][DEL] confirm UI opened", { habitId: id });
    ...
    const executeDelete = (e: React.MouseEvent, id: string) => {
        e.stopPropagation();
        console.debug("[HABITS][DEL] confirmed delete", { habitId: id });
        if (onUpdateHabits) {
            onUpdateHabits(habits.filter(h => h.id !== id));
            console.debug("[HABITS][DEL] archive applied", { archivedCountAfter: habits.length - 1 });
        }
        setDeleteCandidateId(null);
    };
                ...
                {habits.length === 0 ? (
                    ...
                    <div className="space-y-6">
                        ...
                        {habits.map(habit => {
                            ...
                            // WEEK VIEW (Default)
                            // If user clicked trash, show confirmation UI for this item
