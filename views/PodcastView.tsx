...
import { ArrowLeft, Headphones, Play, Pause, Rewind, FastForward, X, Loader, Mic2, ChevronDown, FileText, Download, FolderOpen, Bookmark, Trash2, RefreshCw, Plus, Minus, Volume2 } from 'lucide-react';
import { Card } from '../components/Card';
import { Topic } from '../types';
import { generatePodcastAudio } from "../services/gemini";
import { createWavBlob } from "../utils/audio";
import { getAudioFromIDB, deleteAudioFromIDB, getAllAudioKeys, getTopicBodyFromIDB } from '../services/storage';
...
export const PodcastSettingsView: React.FC<{ 
    config: { language: 'English' | 'Hinglish', voice?: 'Standard' | 'Deep' }; 
    onUpdate: (config: { language: 'English' | 'Hinglish', voice?: 'Standard' | 'Deep' }) => void;
    ...
    return (
        <div className="px-0 py-4 space-y-6">
            ...
            <div className="space-y-4">
                ...
                <Card className="p-4">
                    <h3 className="font-semibold text-gray-800 dark:text-white mb-3">Narrator Voices</h3>
                    <div className="flex space-x-2">
                        {['Standard', 'Deep'].map((voice) => (
                            <div key={voice} className="relative flex-1"><button onClick={() => onUpdate({ ...config, voice: voice as 'Standard' | 'Deep' })} className={`w-full py-3 rounded-xl font-bold transition border-2 ${
                            ...
                            >
                                {previewLoading === voice ? <Loader size={14} className="animate-spin" /> : previewPlaying === voice ? <Pause size={14} /> : <Volume2 size={14} />}</button></div>))}
                    </div>
                    <p className="text-xs text-gray-500 mt-2">
                        Choose the voice tones of your conversational narrators.
                    </p>
                </Card>
                <button 
...
export const PodcastMiniPlayer: React.FC<{
    ...
    return (
        <div 
            className="absolute bottom-[calc(4rem+env(safe-area-inset-bottom))] left-0 right-0 bg-white/95 dark:bg-gray-800/95 backdrop-blur-md rounded-t-2xl shadow-[0_-4px_10px_rgba(0,0,0,0.05)] border-t border-gray-100 dark:border-gray-700 p-2 z-50 flex items-center justify-between cursor-pointer transform transition-all duration-300 pointer-events-auto"
            onClick={() => navigateTo('podcast')}
        >
            {/* Progress Bar Background */}
            <div className="absolute bottom-0 left-0 right-0 h-1 bg-gray-100 dark:bg-gray-700 overflow-hidden pointer-events-none">
                 <div 
...
export const PodcastFullView: React.FC<PodcastFullViewProps> = ({ 
    userId, studyLog, onUpdateTopic, themeColor, onMinimize, state, controls, defaultLanguage
}) => {
    // UI Local State
    ...
    const subjectsMap = useMemo(() => {
        return studyLog.reduce((acc, topic) => {
            ...
            return acc;
        }, {} as Record<string, {name: string, id: string, count: number, topics: Topic[], latestCreatedAt: string}>);
    }, [studyLog]);
    ...
    const availableSubjectNames = useMemo(() => {
        ...
        if (libraryTab === 'downloads') {
            ...
            studyLog.forEach(t => { subjects.add(t.subject); });
        }
    ...
    const handlePlaySubject = async (subjectId: string) => {
        ...
        for (const topic of subjectData.topics) {
            let body = topic.shortNotes || '';
            if (body.trim().length > 0) {
                allNotes += `--- TOPIC: ${topic.topicName} ---\n${body}\n\n`;
            }
        }
