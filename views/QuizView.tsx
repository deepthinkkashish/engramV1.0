...
export const QuizView: React.FC<QuizViewProps> = ({ topic, userId, navigateTo, onUpdateTopic, themeColor }) => {
    ...
    const generateQuiz = useCallback(async (forceRetry = false) => {
        ...
        try {
            ...
            if (data?.pop_quiz?.length === 10) {
                console.debug("[POPQUIZ] success", { questionCount: data.pop_quiz.length });
                setQuizData(data.pop_quiz);
                setStatus('ready');
            ...
            else if (e.message?.includes("quota") || e.name === 'UsageLimitError') msg = e.message || "AI quota reached. Please check Settings.";
    ...
    const submitRepetition = async (score: number, finalTimeTaken: number, fullQuizData: QuizQuestion[], userAnswers: any[]) => {
        ...
        try {
            ...
            console.debug("[POPQUIZ] complete -> showing review");
    ...
    if (status === 'error') return (
        <Card className="py-10 bg-red-50 border border-red-200 text-red-800 dark:bg-red-900/20 dark:border-red-800 dark:text-red-200 text-center">
            <AlertTriangle size={40} className="mx-auto mb-4 text-red-500" />
            <h3 className="text-xl font-bold mb-2">Quiz Generation Failed</h3>
            <p className="max-w-xs mx-auto mb-6 text-sm">{errorMessage || "Something went wrong."}</p>
            
            <div className="flex justify-center space-x-3">
                <button 
                    onClick={() => navigateTo('topicDetail', topic)} 
                    className="px-5 py-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-xl font-bold hover:bg-gray-50 dark:hover:bg-gray-700 transition"
                >
                    Back
                </button>
                {errorMessage?.includes("limit") ? (
                    <button 
                        ...
                        className={`px-5 py-2.5 bg-${themeColor}-600 text-white rounded-xl font-bold hover:bg-${themeColor}-700 transition flex items-center shadow-lg active:scale-95`}
                    >
                        Go to Settings
                    </button>
                ) : (
        ...
        <Card className="bg-white dark:bg-gray-800 p-4 md:p-6 flex flex-col flex-1 w-full min-h-0 shadow-sm overflow-hidden">
            <div className="shrink-0 mb-4 pb-2 border-b dark:border-gray-700">
                <div className="flex justify-between items-center">
                    <div>
                        <h3 className={`text-lg md:text-xl font-bold text-${themeColor}-700 dark:text-${themeColor}-300 truncate mr-2`}>Pop Quiz: {topic?.topicName}</h3>
                        {isModelFallback && (
                            <span className="text-[10px] text-orange-500 font-medium flex items-center">
                                <AlertTriangle size={10} className="mr-1"/> Preview limits reached; switched to Flash.
                            </span>
                        )}
                    </div>
                    <div className="flex items-center space-x-3 shrink-0">
                        <p className={`text-base md:text-lg font-mono font-bold text-${themeColor}-500 dark:text-${themeColor}-400`}><Timer size={16} className="inline mr-1 mb-0.5" />{timeTaken}s</p>
                        <p className="text-xs md:text-sm font-medium text-gray-600 dark:text-gray-400">{currentQuestionIndex + 1} / 10</p>
                    </div>
                </div>
            </div>
            <div className="flex-1 overflow-y-auto pr-1 -mr-1 custom-scrollbar min-h-0 relative">
                ...
                <div className="space-y-3 pb-2">
                    {Object.entries(currentQ.options).map(([letter, text]) => (
                        <button key={letter} onClick={() => handleAnswer(letter)} className={`w-full text-left p-3 md:p-4 border border-gray-200 dark:border-gray-700 rounded-xl hover:bg-${themeColor}-50 dark:hover:bg-${themeColor}-900/30 transition duration-150 flex items-start space-x-3 shadow-sm active:scale-[0.98] group`}>
