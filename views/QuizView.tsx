...
export const QuizView: React.FC<QuizViewProps> = ({ topic, userId, navigateTo, onUpdateTopic, themeColor, onLockTabs }) => {
    ...
    const generateQuiz = useCallback(async (forceRetry = false) => {
        ...
        try {
            ...
            // 2. Build Context (Difficulty, Mistakes, Mastered)
            ...
            let difficultyContext = "Difficulty Level: 1/5 (Foundation). Focus on factual recall, basic definitions, and core principles directly from the text.";
            if (repetitionLevel === 1) {
                difficultyContext = "Difficulty Level: 2/5 (Comprehension). Require understanding of concepts, not just rote memorization. Use minor scenario-based questions.";
            } else if (repetitionLevel === 2) {
                difficultyContext = "Difficulty Level: 3/5 (Application). Require application of concepts to new situations or solving moderate problems based on the notes.";
            } else if (repetitionLevel === 3) {
                difficultyContext = "Difficulty Level: 4/5 (Analysis). Require critical thinking, comparing/contrasting concepts, and identifying underlying structures.";
            } else if (repetitionLevel >= 4) {
                difficultyContext = "Difficulty Level: 5/5 (Mastery/Synthesis). Highly challenging. Require synthesizing multiple concepts, evaluating complex scenarios, and deep critical reasoning.";
            }
            ...
            const recentMistakes = [...new Set(allMistakes)].slice(-15);
            let mistakeContext = "";
            if (recentMistakes.length > 0) {
                mistakeContext = `\n\n--- WEAKNESSES (TARGET THESE) ---\nThe user previously struggled with these concepts. Prioritize generating NEW, challenging questions targeting these specific areas to test if they have learned from their mistakes:\n- ${recentMistakes.join('\n- ')}`;
            }

            // Mastered Analysis (Avoid Repetition)
            const allCorrect = fullTopic.repetitions?.flatMap(r => 
                r.quizAttempt.questions.filter(q => {
                    const selected = q.userSelected;
                    const correct = q.correctAnswer || q.correct_answer_letter;
            ...
            const recentCorrect = [...new Set(allCorrect)].slice(-30);
            let correctContext = "";
            if (recentCorrect.length > 0) {
                correctContext = `\n\n--- MASTERED CONCEPTS (AVOID REPEATING) ---\nCRITICAL CONSTRAINT: The user has already correctly answered the following questions in past sessions. Do NOT generate questions that are identical or highly similar to these. Force yourself to test other areas of the notes or much deeper nuances:\n- ${recentCorrect.join('\n- ')}`;
            }
