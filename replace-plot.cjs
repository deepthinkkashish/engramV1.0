...
const path = 'components/PlotComponent.tsx';
...
export const PlotComponent: React.FC<PlotComponentProps> = ({ data, title, xAxisLabel, yAxisLabel }) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm my-4 touch-pan-x touch-pan-y border border-gray-100 dark:border-gray-700">
            {title && <h3 className="text-sm font-bold text-gray-800 dark:text-gray-100 mb-4 text-center">{title}</h3>}
            <svg ref={svgRef} viewBox="0 0 400 330" className="w-full h-auto text-gray-600 dark:text-gray-400" />
        </div>
    );
};
\`;
